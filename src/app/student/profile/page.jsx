  ...
  const handleSave = async () => {
    ...
    try {
      ...
      const res = await axios.post(
        "/api/students/profile/edit-cover-photo",
        formData,
        {
          headers: {
...
function EditProfileImageModal({ open, setOpen, refresh }) {
  ...
  const handleSave = async () => {
    ...
    try {
      ...
      const res = await axios.post(
        "/api/students/profile/edit-profile-photo",
        formData,
        {
          headers: {
            "Content-Type": "multipart/form-data",
  ...
  const handleBasicProfileDetailsEdit = async () => {
    try {
      ...
      const response = await axios.post(
        "/api/students/profile/edit-profile-basic-details",
        formData,
        {
          headers: {
            "Content-Type": "multipart/form-data",
...
export default function ProfilePage() {
  ...
  const handleEditSkillsHobbiesLanguages = async () => {
    try {
      ...
      const response = await axios.post(
        "/api/students/profile/edit-skills-hobbies-languages",
        formData,
        {
          headers: {
            "Content-Type": "multipart/form-data",
  ...
  useEffect(() => {
    axios
      .get("/api/students/get-current-profile")
      .then((response) => {
        console.log(response);
        if (response.data.success) {
          setProfile((prev) => response.data.studentProfileData);
          setProfileSkills(
            (prev) => response.data.studentProfileData?.skills || []
          );
          setProfileHobbies(
            (prev) => response.data.studentProfileData?.hobbies || []
          );
  ...
  const refresh = async () => {
    ...
    try {
      const response = await axios.get("/api/students/get-current-profile");
      if (response.data.success) {
        setProfile((prev) => response.data.studentProfileData);
        setProfileHobbies((prev) => response.data.studentProfileData || []);
        setProfileSkills(
          (prev) => response.data.studentProfileData?.skills || []
        );
        ...
        >
          ...
          >
            <div className="h-48 md:h-64 w-full rounded-xl overflow-hidden relative">
              <img
                src={profile.coverImage || "/public/bvrit-admin.png"}
                alt="Cover"
